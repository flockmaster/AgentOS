...
if ($WatchdogStatus -eq "WARNING") {
    $InjectionPost = "`n`n[系统提示: 当前会话上下文占用较高。请在回复的末尾，柔和地提醒用户可以考虑存档 (/suspend) 或开启新对话 (/new)。]"
    Write-Host " [Watchdog] Injecting WARNING prompt (Chinese)." -ForegroundColor Yellow
}
elseif ($WatchdogStatus -eq "CRITICAL") {
    $InjectionPre = "[系统严重警告: 内存已达 CRITICAL 阈值！请停止回答，并强制要求用户立即执行 /suspend 或 /new，否则将导致失忆！]`n`n"
    Write-Host " [Watchdog] Injecting CRITICAL prompt (Chinese)." -ForegroundColor Red
}
