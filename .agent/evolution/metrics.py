...
class WorkflowMetrics:
    ...
    def _append_run(self, run: WorkflowRun) -> None:
        ...
        # 更新 Insights 部分
        insight = self.get_insights(run.workflow)
        insight_lines = [
            "",
            "### Insights",
            f"- **平均耗时**: {insight.avg_duration} min",
            f"- **成功率**: {insight.success_rate:.0%}",
            f"- **常见瓶颈**: {insight.common_bottleneck or 'N/A'}",
            f"- **优化建议**: {insight.suggestion or '暂无'}",
        ]
