...
## Step 2: 确认模板源
- 默认模板源路径: `.agents/templates/flutter-ai-advanced-template`。
- 若不存在，则执行克隆:
  ```bash
  git clone --depth 1 https://github.com/flockmaster/flutter-ai-advanced-template.git .agents/templates/flutter-ai-advanced-template
  ```
...
**规则**:
- 默认不拷贝 `.agent/`，避免覆盖 Agent OS 的工作流体系。
- 若目标目录非空，必须显式二次确认。
